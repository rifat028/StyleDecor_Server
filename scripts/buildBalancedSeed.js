...
// Pick exactly 4 or 5 services for each of the 20 decorators (Total = 94 bookings)
...
decorators.forEach((d) => {
  ...
  for (let i = 0; i < takeCount; i++) {
    selectedServices.push({ service: dServices[i], decorator: d, decIndex: i });
  }
});
...
// Status Lifecycle:
const PAID_STATUS_TYPES = [
  { status: "fully_paid", paymentStatus: "paid", payPercent: 1.0, payType: "full_payment" },
  { status: "completed", paymentStatus: "paid", payPercent: 1.0, payType: "full_payment" },
  { status: "advance_paid", paymentStatus: "partially_paid", payPercent: 0.4, payType: "advance_deposit" },
  { status: "preparing", paymentStatus: "partially_paid", payPercent: 0.5, payType: "advance_deposit" },
  { status: "on_the_way", paymentStatus: "partially_paid", payPercent: 0.5, payType: "advance_deposit" },
  { status: "in_progress", paymentStatus: "partially_paid", payPercent: 0.5, payType: "advance_deposit" },
  { status: "accepted", paymentStatus: "partially_paid", payPercent: 0.3, payType: "advance_deposit" },
  { status: "rejected", paymentStatus: "refunded", payPercent: 1.0, payType: "full_payment", isCancelled: true, reason: "Client requested cancellation due to venue schedule change." },
];

const UNPAID_STATUS_TYPES = [
  { status: "pending", paymentStatus: "unpaid", payPercent: 0.0, payType: "unpaid" },
  { status: "in_draft", paymentStatus: "unpaid", payPercent: 0.0, payType: "unpaid" },
];

// Track unpaid allocations per customer and per decorator
const customerUnpaidCount = {};
customers.forEach((c) => (customerUnpaidCount[c._id.toString()] = 0));
const decoratorUnpaidCount = {};
decorators.forEach((d) => (decoratorUnpaidCount[d._id.toString()] = 0));
...
selectedServices.forEach((item, index) => {
  const { service, decorator, decIndex } = item;
  ...
  const dId = decorator._id.toString();

  const decAgents = agentsByDec[dId] || [];
  ...
  // Decide if this booking has a payment or is unpaid:
  // A booking can be unpaid ONLY IF both the customer and decorator currently have 0 unpaid bookings!
  let isUnpaid = false;
  if (decIndex === 3 && decoratorUnpaidCount[dId] === 0 && customerUnpaidCount[cId] === 0) {
    isUnpaid = true;
    decoratorUnpaidCount[dId]++;
    customerUnpaidCount[cId]++;
  }

  let statusCfg;
  if (!isUnpaid) {
    statusCfg = PAID_STATUS_TYPES[paidIdx % PAID_STATUS_TYPES.length];
    paidIdx++;
  } else {
    statusCfg = UNPAID_STATUS_TYPES[unpaidIdx % UNPAID_STATUS_TYPES.length];
    unpaidIdx++;
  }

  const hasPayment = !isUnpaid;
  const paidAmount = hasPayment ? Math.round(grandTotal * statusCfg.payPercent) : 0;
  ...
  const booking = {
    ...
    assignedAgentId:
      statusCfg.status === "in_draft" || statusCfg.status === "pending"
        ? null
        : assignedAgent
        ? assignedAgent._id
        : null,
  ...
  if (hasPayment) {
    ...
    const payMethod = ['bkash', 'nagad', 'sslcommerz', 'bank_transfer', 'stripe'][paymentCounter % 5];
    const payAmt = paidAmount > 0 ? paidAmount : grandTotal;

    const gatewayFee = Math.round(payAmt * 0.015);
    const platformCommission = Math.round(payAmt * 0.10);
    const vendorReceivable = payAmt - gatewayFee - platformCommission;
